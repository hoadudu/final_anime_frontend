    ...
    <section class="latest-episode-section q-py-lg">
        <div class="section-header row items-center justify-between q-mb-lg">
            <div class="section-title">
                <h2 class="text-h4 text-weight-bold text-white q-mb-none">{{ t('latestEpisodes.title') }}</h2>
            </div>
            <q-btn flat color="primary" icon-right="chevron_right" class="view-more-btn text-weight-medium" no-caps
        ...
        <!-- Empty State -->
        <div v-else-if="!animeList || animeList.length === 0" class="empty-container text-center q-py-xl">
        ...
        <div v-else class="anime-posts-grid">
            <div class="anime-posts-container">
                <div v-for="(animePost, index) in animeList" :key="animePost.id || `anime-${index}`"
                    class="anime-post-item" @click="navigateToAnime(animePost)">
                    <q-card flat bordered class="anime-card">
                        <div class="anime-poster-container">
                            <!-- Rating Badge -->
                            <q-badge v-if="animePost.ageRating === '18+'" floating color="red" class="age-rating-badge">
                                18+
                            </q-badge>

                            <!-- Episode Info Badges -->
                            <div class="anime-badges">
                                <q-badge v-if="animePost.subCount" color="blue" text-color="white" class="anime-badge">
                                    <q-icon name="closed_caption" size="12px" class="q-mr-xs" />
                                    {{ animePost.subCount }}
                                </q-badge>

                                <q-badge v-if="animePost.dubCount" color="green" text-color="white"
                                    class="anime-badge q-ml-xs">
                                    <q-icon name="mic" size="12px" class="q-mr-xs" />
                                    {{ animePost.dubCount }}
                                </q-badge>

                                <q-badge v-if="animePost.totalEpisodes" color="orange" text-color="white"
                                    class="anime-badge q-ml-xs">
                            ...
                            <div class="anime-poster-wrapper">
                                <q-img :src="animePost.poster || animePost.image || animePost.thumbnail"
                                    :alt="animePost.title || animePost.name" fit="cover" class="anime-poster"
                                    loading="lazy" spinner-color="primary" @error="handleImageError">
        ...
        <q-btn v-if="!isLoading && animeList && animeList.length > 0" fab color="primary" icon="refresh"
            class="refresh-fab" @click="refreshLatestAnimePosts" :loading="isRefreshing" />
    </section>
</template>
