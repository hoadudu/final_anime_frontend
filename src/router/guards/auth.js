  ...
  return async function authGuard(to, from, next) {
    ...
    if (hasValidRefreshToken) {
      try {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.log('🔍 Auth guard debug after refresh:', {
            hasUser: !!authStore.user,
            isAuthenticated: authStore.isAuthenticated,
            hasValidToken: authStore.hasValidToken,
            userInStorage: !!tokenStorage.getUser(),
            tokenInStorage: !!tokenStorage.getAccessToken(),
          })
        }

        // If user data is still not available after refresh, try to load profile
        if (!authStore.user && authStore.hasValidToken) {
          if (process.env.NODE_ENV === 'development') {
          ...
          try {
            ...
            if (process.env.NODE_ENV === 'development') {
              console.warn('⚠️ Auth guard: Failed to load profile:', profileError.message)
            }
          }
        }

        // Check if refresh was successful - Simplified check
        if (authStore.user && authStore.hasValidToken) {
          if (process.env.NODE_ENV === 'development') {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.log('⚠️ Auth guard: After refresh check failed:', {
            hasUser: !!authStore.user,
            hasValidToken: authStore.hasValidToken,
          })
        }
